...
public class AppConfig : IDisposable
{
    ...
    private void SingleInstanceService_OnNewInstance(object sender, SingleInstanceEventArgs message)
    {
        ...
        {
            if (message.Arguments[i].Equals(Statics.Global.StringConstants.ProfileAutoStartArgument, StringComparison.Ordinal) &&
    ...
    private string? GetArgumentProfileId(StartupEventArgs startupEventArgs)
    {
        ...
        {
            if (!startupEventArgs.Args[i].Equals(Statics.Global.StringConstants.ProfileAutoStartArgument, StringComparison.Ordinal)) continue;
